...
def execute(in_dir, delim, out_file, age_gender=False, counts=False, enriched_counts=False,
				tmprl=False, enriched_tmprl=False):
	'''merge the in files to produce the out file'''
	merged = defaultdict(list)
	headers = ['ID']

	if age_gender:
		headers = merge_file(in_dir+'/age+gender.csv', merged, headers, delim)

	if counts:
		headers = merge_file(in_dir+'/counts.csv', merged, headers, delim)

	if enriched_counts:
		headers = merge_file(in_dir+'/counts_enriched.csv', merged, headers, delim)

	if tmprl:
		headers = merge_file(in_dir+'/temporal.csv', merged, headers, delim)

	if enriched_tmprl:
		headers = merge_file(in_dir+'/tmprl_enriched.csv', merged, headers, delim)

	headers = merge_file(in_dir+'/target.csv', merged, headers, delim)
